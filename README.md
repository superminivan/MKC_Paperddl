...
## 可选：使用 Docker 运行 MySQL（仅数据库）

如果不想在本机安装 MySQL，可用 Docker Compose 启动一个最小的 MySQL 服务：

```yaml
services:
  mysql:
    image: mysql:8.0
    container_name: paperddl-mysql
    environment:
    ...
    ports:
      - "3306:3306"
    volumes:
      - paperddl-mysql-data:/var/lib/mysql
...
启动后在容器中或通过主机的 MySQL 客户端运行迁移并创建管理员。

---

如果你希望我把 README 再进一步细化（例如：添加 Windows 下 PowerShell 逐步导入 SQL 的截图、或把 Docker Compose 写成完整的 repo-level `docker-compose.yml`），告诉我要补充哪些部分，我会继续完善。 
